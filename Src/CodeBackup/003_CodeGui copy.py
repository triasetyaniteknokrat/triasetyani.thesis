...
- Buttons: Load Model, Open Media, Open Camera, Reset Position, Stop, CLAHE, REC
- Camera mode:
...
BUTTON_WIDTH = 16  # Tk units (text-based)
BUTTON_HEIGHT = 2
...
def view_to_rect(view: ViewState, canvas_w: int, canvas_h: int, img_w: int, img_h: int):
    """Return visible rect (x1,y1,x2,y2) in IMAGE coordinates based on current view transform."""
...
# ---------------- ImageCanvas (zoom/pan like ImageLabel) ----------------
class ImageCanvas(tk.Canvas):
    """
    Canvas that displays an RGB image and supports zoom (wheel) + pan (drag),
    using a view transform (scale + offset). It also exposes current view state.
    """
    def __init__(self, master, width=SQUARE, height=SQUARE, **kwargs):
        super().__init__(master, width=width, height=height, highlightthickness=1, highlightbackground="#444", **kwargs)
    ...
    def _redraw(self):
        ...
        # Create "canvas-sized" black background, then paste scaled image at offsets
        cw = int(self.cget("width"))
        ch = int(self.cget("height"))
    ...
    def _on_wheel(self, e):
        # e.delta: 120/-120 on Windows, smaller on Mac
        delta = e.delta
...
class InferenceWorker(threading.Thread):
    """
    Thread that performs inference in camera/video/image mode and pushes results into a queue.
    """
    def __init__(self, model: YOLO, device: str, use_clahe: bool, clahe_clip: float, clahe_tile: int,
                 project_root: Path, out_queue: queue.Queue):
        ...
        # for image mode: trigger re-infer
        self._trigger = threading.Event()
        self._cached_image_sq = None  # BGR square
        self._cached_orig_rgb = None  # RGB square
    ...
    def _start_recording(self):
        ...
        header = ["frame_idx", "timestamp", "inference_ms", "fps_inst", "clahe_on"]
    ...
    def _process_one_frame(self, frame_sq_bgr: np.ndarray):
        ...
        img_proc = self._apply_clahe(img_for_infer) if self.use_clahe else img_for_infer
        ...
        # push frame to GUI
        self.out_queue.put(("frame", orig_rgb, full_annot_rgb, counts, dt, self._mode))
        ...
        if self._recording and self._csv_writer is not None:
            ...
            row = [self._frame_idx, ts_ms(), round(inference_ms, 3), round(fps_inst, 3), clahe_on]
            total_objects = 0
            for cn in self.class_names:
    ...
    def run(self):
        ...
        try:
            if self._mode == "camera":
                ...
                self._cached_orig_rgb = cv2.cvtColor(frame_sq, cv2.COLOR_BGR2RGB)
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Project root: .../DORISJUARSA.THESIS (sesuaikan seperti kode kamu: parents[2])
        # Pastikan file ini diletakkan di: Src/CodeTesting/xxx.py  -> root = parents[2]
        ...
        self.btn_load = tk.Button(btn_frame, text="Load Model", width=BUTTON_WIDTH, height=BUTTON_HEIGHT, command=self.load_model)
        self.btn_media = tk.Button(btn_frame, text="Open Media", width=BUTTON_WIDTH, height=BUTTON_HEIGHT, command=self.open_media, state="disabled")
        self.btn_cam = tk.Button(btn_frame, text="Open Camera", width=BUTTON_WIDTH, height=BUTTON_HEIGHT, command=self.open_camera, state="disabled")
        self.btn_reset = tk.Button(btn_frame, text="Reset Position", width=BUTTON_WIDTH, height=BUTTON_HEIGHT, command=self.reset_views, state="disabled")
        self.btn_stop = tk.Button(btn_frame, text="Stop", width=BUTTON_WIDTH, height=BUTTON_HEIGHT, command=self.stop_worker, state="disabled")

        self.clahe_var = tk.BooleanVar(value=False)
        self.btn_clahe = tk.Checkbutton(
            btn_frame, text="CLAHE: OFF", width=BUTTON_WIDTH, height=BUTTON_HEIGHT,
            variable=self.clahe_var, command=self.toggle_clahe,
            indicatoron=False, selectcolor="#d0ffd0", state="disabled"
        )

        self.rec_var = tk.BooleanVar(value=False)
        self.btn_rec = tk.Checkbutton(
            btn_frame, text="REC: OFF", width=BUTTON_WIDTH, height=BUTTON_HEIGHT,
            variable=self.rec_var, command=self.toggle_rec,
            indicatoron=False, selectcolor="#ffd0d0", state="disabled"
        )

        for w in [self.btn_load, self.btn_media, self.btn_cam, self.btn_reset, self.btn_stop, self.btn_clahe, self.btn_rec]:
        ...
        # when left view changes -> update crop rect + sync right view
    ...
    def _enable_after_model(self, enable=True):
        ...
        for w in [self.btn_media, self.btn_cam, self.btn_reset, self.btn_stop, self.btn_clahe, self.btn_rec]:
            w.configure(state=state)

    def _set_clahe_ui(self, is_on: bool):
        self.clahe_var.set(is_on)
        self.btn_clahe.configure(text=("CLAHE: ON" if is_on else "CLAHE: OFF"))

    def _set_rec_ui(self, is_on: bool):
        self.rec_var.set(is_on)
        self.btn_rec.configure(text=("REC: ON" if is_on else "REC: OFF"))
    ...
    def load_model(self):
        ...
        try:
            ...
            clahe_status = "CLAHE ON" if self.clahe_var.get() else "CLAHE OFF"
            self.status_var.set(f"Model dimuat: {Path(path).name} ({self.device}) | {clahe_status} | conf={DEFAULT_CONF}")
    ...
    def toggle_clahe(self):
        is_on = bool(self.clahe_var.get())
        self.btn_clahe.configure(text=("CLAHE: ON" if is_on else "CLAHE: OFF"))

        if self.worker:
            self.worker.use_clahe = is_on

        self.status_var.set("CLAHE ON" if is_on else "CLAHE OFF")

    # ---------- REC ----------
    def toggle_rec(self):
        is_on = bool(self.rec_var.get())
        ...
        self.btn_rec.configure(text=("REC: ON" if is_on else "REC: OFF"))
        self.status_var.set("REC ● Recording started..." if is_on else "REC ■ Recording stopped (preview continues)")
    ...
    def open_camera(self):
        ...
        cam_str = simpledialog.askstring("Pilih Kamera", f"Masukkan index kamera {cams} (contoh: {cams[0]}):")
        if cam_str is None:
            return
        try:
            cam_index = int(cam_str.strip())
        except Exception:
            messagebox.showerror("Input salah", "Index kamera harus angka.")
            return
        if cam_index not in cams:
            messagebox.showerror("Input salah", f"Index kamera tidak tersedia. Terdeteksi: {cams}")
            return
    ...
    # ---------- Queue polling (frames, errors, session events) ----------
    def _poll_queue(self):
        try:
            while True:
                ...
                if kind == "frame":
                    _, orig_rgb, ann_rgb, counts, dt, mode = item
                    self._update_canvases(orig_rgb, ann_rgb)
    ...
    def _update_canvases(self, orig_rgb, ann_rgb):
        # left and right keep their own view transforms.
        # For smoothness, do not reset view on every frame.
...
def main():
    # optional: reduce OpenCV threading issues
    try:
        cv2.setNumThreads(0)
    except Exception:
        pass
